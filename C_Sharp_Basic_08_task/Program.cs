    ...
    class Program
    {
        static void Main(string[] args)
        {
...

            {
                ...
                List<string> str = new List<string> { "1qwerty", "cqwertyc", "cqwe", "7qwe", "c" };
                int sumLength = str.Sum(x => x.Length);
            }

            {
                ...
                List<string> str = new List<string> { "1qwerty", "2qwerty", "cqwe", "7qwe" };
...

            {
                ...
                int K = 4;
                List<string> A = new List<string> { "QWERTY1", "QWERTY2", "QWE7", "QWE4" };
...

            {
                ...
                int D = 33, K = 5;
                IEnumerable<int> A = new int[] { 12, 32, 33, 34, 88, 1, 3, 5, 4, 6, 6, 2, 5, 8, 9, 0, 90 };
...

            {
                ...
                List<string> str = new List<string> { "QWERTY1", "QWERTY2", "QWE7", "QWE4" };
                var res = string.Concat(str.Select(x => x.Length % 2 != 0 ? x[0] : x[x.Length - 1])).OrderByDescending(x => x);
            }

            {
                ...
                var res = A.Where(x => x > K1).Union(B.Where(y => y < K2)).OrderBy(x => x);
            }
            {
            ...
            }
            {
                ...
                List<string> A = new List<string> { "ABCDEF1", "ABCDEF2", "ACC88", "ABC4" };
                List<string> B = new List<string> { "ABCDEF11", "ABCDEF22", "AAC7", "ABCC", "ABCDD" };
                var res = A.Join(B, x => x.Length, y => y.Length, (x, y) => (x, y)).OrderBy(x => x).ThenByDescending(y => y);
...

            {
...

                {
                    ...
                    {
                        new Applicant { SchoolNumber = 58, EntryYear = 2018, Surname = "Washington" },
                        new Applicant { SchoolNumber = 35, EntryYear = 2018, Surname = "Gonzales" },
                        new Applicant { SchoolNumber = 61, EntryYear = 2017, Surname = "Mitchell" },
                        new Applicant { SchoolNumber = 82, EntryYear = 2017, Surname = "Edwards" },
                        new Applicant { SchoolNumber = 11, EntryYear = 2017, Surname = "Campbell" },
                        new Applicant { SchoolNumber = 30, EntryYear = 2017, Surname = "Walker" },
                        new Applicant { SchoolNumber = 95, EntryYear = 2018, Surname = "Bennett" },
                        new Applicant { SchoolNumber = 77, EntryYear = 2018, Surname = "Bryant" }
                    };

                    foreach (var applicantGroup in applicants.GroupBy(applicantGroup => applicantGroup.EntryYear)
                        .Select(applicantGroups => new { EntryYear = applicantGroups.Key, Count = applicantGroups.Count() })
                        .OrderBy(applicantGroups => applicantGroups.Count).ThenBy(applicantGroups => applicantGroups.EntryYear))
                        Console.WriteLine($"Amount of schools={applicantGroup.Count}, Entry year={applicantGroup.EntryYear}");
                }
            }
        }
    }
